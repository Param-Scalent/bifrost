...
request_type   // Request type (chat_completion, list_models, embedding, batch, image_generation, moderation, transcription, translation)
```
...
// Route model-list requests to a specific provider and key
request_type == "list_models"

// Example target: provider=openrouter, key_id=provider-key-uuid
